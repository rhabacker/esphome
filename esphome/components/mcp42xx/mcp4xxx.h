...
class MCP4XXX {
 public:
  ...
  enum Pot { pot_0 = B00, pot_1 = B01 };

  // The resolution of the device's resistor ladder.
  enum Resolution { res_7bit = 127, res_8bit = 255 };

  // Whether the device is a rheostat (MCP4XX2), or potentiometer (MCP4XX1).
  enum WiperConfiguration { rheostat = 0, potentiometer = 1 };

  /**
   * select_pin  - The pin to use for the SPI slave select signal, defaults to the main slave select pin on your
   * Arduino. pot         - For the 2-pot variants (MCP42XX), the potentiometer to control. Must be pot_0 for MCP41XX
   * chips. resolution  - res_7bit for MCP4X3X and MCP4X4X, res_8bit for MCP4X5X and MCP4X6X.
   */
  MCP4XXX(byte select_pin = SS, Pot pot = pot_0, Resolution resolution = res_8bit,
          WiperConfiguration config = potentiometer);
   ...
   *
   * The higher value on potentiometers (MCP4XX1) facilitates direct connection of the wiper to the "A" terminal (also
   * known as "full-scale"). Confusingly the rheostat devices (MCP4XX2) have only a wiper pin and "B" terminal pin, (not
   * "A").
   */
 ...
 private:
  ...
  enum Address { address_pot0_wiper = B0000, address_pot1_wiper = B0001, address_tcon = B0100, address_status = B0101 };

  enum Command { command_write = B00, command_read = B11, command_increment = B01, command_decrement = B10 };
