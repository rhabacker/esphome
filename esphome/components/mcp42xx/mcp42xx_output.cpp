...
static const char *const TAG = "MCP42XX";

void MCP42XXComponent::setup() {
  delay(10);
  dump_config();
}

void MCP42XXComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "config:");
  ESP_LOGI(TAG, "  Maximum value: %d", max_value());
}

void MCP42XXComponent::loop() {}

void MCP42XXComponent::set_channel_value_(MCP42XXChannelIdx channel, uint16_t value) {
  ESP_LOGI(TAG, "Writing value %3d to poti %d", value, 0);
  set(value);
}

// 0..1 []
void MCP42XXChannel::write_state(float state) {
...
uint32_t MCP42XXChannel::hash_base() { return 2455723295UL; }
