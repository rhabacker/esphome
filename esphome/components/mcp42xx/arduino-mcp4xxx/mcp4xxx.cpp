...
MCP4XXX::MCP4XXX(byte select_pin, Pot pot, Resolution resolution, WiperConfiguration config)
    : m_select_pin(select_pin),
      m_pot(pot),
      m_max_value(resolution + config)  // Potentiometer configurations allow "resolution + 1" values, for setting the
                                        // "full-scale" wiper position.
      ,
      m_select_nesting(0) {
  SPI.begin();
...
word MCP4XXX::max_value(void) const { return m_max_value; }

bool MCP4XXX::increment(void) { return transfer(static_cast<Address>(m_pot), command_increment); }

bool MCP4XXX::decrement(void) { return transfer(static_cast<Address>(m_pot), command_decrement); }

bool MCP4XXX::set(word value) { return transfer(static_cast<Address>(m_pot), command_write, min(value, m_max_value)); }
...
bool MCP4XXX::set_terminal_a_status(bool connected) { return set_tcon(tcon_term_a_mask, connected); }

bool MCP4XXX::get_terminal_a_status(bool &connected) const { return get_tcon(tcon_term_a_mask, connected); }

bool MCP4XXX::get_terminal_a_status(void) const {
  bool connected = false;
  return get_terminal_a_status(connected) && connected;
}

bool MCP4XXX::set_terminal_b_status(bool connected) { return set_tcon(tcon_term_b_mask, connected); }

bool MCP4XXX::get_terminal_b_status(bool &connected) const { return get_tcon(tcon_term_b_mask, connected); }

bool MCP4XXX::get_terminal_b_status(void) const {
  bool connected = false;
  return get_terminal_b_status(connected) && connected;
}

bool MCP4XXX::set_wiper_status(bool connected) { return set_tcon(tcon_wiper_mask, connected); }

bool MCP4XXX::get_wiper_status(bool &connected) const { return get_tcon(tcon_wiper_mask, connected); }

bool MCP4XXX::get_wiper_status(void) const {
  bool connected = false;
  return get_wiper_status(connected) && connected;
}

bool MCP4XXX::set_shutdown_status(bool shutdown) { return set_tcon(tcon_shutdown_mask, !shutdown); }
...
bool MCP4XXX::get_shutdown_status(void) const {
  bool shutdown = false;
  return get_shutdown_status(shutdown) && shutdown;
}
...
bool MCP4XXX::get_hardware_shutdown_status(void) const {
  bool shutdown = false;
  return get_hardware_shutdown_status(shutdown) && shutdown;
}
...
byte MCP4XXX::build_command(Address address, Command command) const {
  return ((address << 4) & address_mask) | ((command << 2) & command_mask) | cmderr_mask;
}

word MCP4XXX::build_command(Address address, Command command, word data) const {
  return (build_command(address, command) << 8) | (data & data_mask_word);
}
...
bool MCP4XXX::transfer(Address address, Command command, word data, word &result) const {
  ...
  if (valid) {
    result = ((high_byte & data_mask) << 8) | SPI.transfer(lowByte(cmd));
  }
